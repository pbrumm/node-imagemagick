...
///////////////////////////////////
//   same test as above with a png
var path2 = 'sample-images/test.png';
imdata = fs.readFileSync(path2, 'binary');

im.identify(path2, function(err, features){
  if (err) return sys.error(err.stack || err);
  sys.puts('features: '+sys.inspect(features));
})
...
im.readMetadata(path2, function(err, metadata){
  if (err) return sys.error(err.stack || err);
  sys.puts('metadata: '+sys.inspect(metadata));
})
...
im.resize({
  srcPath: path,
  dstPath: path+'.resized.png',
  width: 256
}, function(err, stdout, stderr){
  if (err) return sys.error(err.stack || err);
  sys.puts('real time taken for convert: '+((new Date)-timeStarted)+' ms')
  im.identify(['-format', '%b', path+'.resized.png'], function(err, r){
    if (err) throw err;
    sys.puts('size: '+r.substr(0,r.length-2)+' Bytes');
  })
})
...
im.resize({
  ...
  if (err) return sys.error(err.stack || err);
  sys.puts('real time taken for convert (with buffers): '+((new Date)-timeStarted)+' ms');
  fs.writeFileSync(path+'.resized-io.png', stdout, 'binary');
  sys.puts('size: '+stdout.length+' Bytes');
})
